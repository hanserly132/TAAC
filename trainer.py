...
class PCVRHyFormerRankingTrainer:
    ...
    def __init__(
        ...
        self.feature_stats: Optional[Dict[str, Any]] = feature_stats
    ...
    def _write_sidecar_files(self, ckpt_dir: str) -> None:
        ...
        - ``feature_stats.pkl`` when engineered item-stat features are enabled:
          train-only aggregate statistics needed to build identical features
          during inference.
        """
        ...
        if self.feature_stats:
            with open(os.path.join(ckpt_dir, 'feature_stats.pkl'), 'wb') as f:
                pickle.dump(self.feature_stats, f, protocol=pickle.HIGHEST_PROTOCOL)
    ...
    def _train_step(self, batch):
        ...
        # 在 _train_step 的 return 之前加，只打印第一个 step
        if not hasattr(self, '_mem_logged'):
            allocated = torch.cuda.memory_allocated() / 1024**3
            reserved = torch.cuda.memory_reserved() / 1024**3
            max_allocated = torch.cuda.max_memory_allocated() / 1024**3
            logging.info(f"[GPU Memory] allocated={allocated:.2f}GiB, "
                         f"reserved={reserved:.2f}GiB, "
                         f"peak_allocated={max_allocated:.2f}GiB")
