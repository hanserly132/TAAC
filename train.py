...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument('--use_engineered_features', action='store_true', default=True,
                        help='Append engineered sequence-time, item-stat and pair dense features '
                             'to user_dense_feats (default on).')
    parser.add_argument('--no_engineered_features', dest='use_engineered_features',
                        action='store_false',
    ...
    parser.add_argument('--compile_model', action='store_true', default=False,
                        help='Enable torch.compile for the model. Disabled by default '
                             ...
                             'benchmark allocations on the competition platform.')
    ...
    if args.data_dir is None:
        raise EnvironmentError("TRAIN_DATA_PATH must be set, or pass --data_dir.")
    if args.ckpt_dir is None:
        raise EnvironmentError("TRAIN_CKPT_PATH must be set, or pass --ckpt_dir.")
    if args.log_dir is None:
        args.log_dir = os.path.join(args.ckpt_dir, 'logs')
    if args.tf_events_dir is None:
        args.tf_events_dir = os.path.join(args.ckpt_dir, 'tf_events')
...
def main() -> None:
    ...
    try:
        ...
        logging.warning(f"TensorBoard SummaryWriter unavailable, scalar logging disabled: {exc}")
        writer = None
    ...
    if args.compile_model:
        logging.info("torch.compile enabled by --compile_model")
        model = torch.compile(model)
    else:
        logging.info("torch.compile disabled; using eager model")
    ...
    trainer = PCVRHyFormerRankingTrainer(
        ...
        feature_stats=pcvr_dataset.feature_stats if args.use_engineered_features else None,
    )
