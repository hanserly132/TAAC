...
from dataset import FeatureSchema, PCVRParquetDataset, NUM_TIME_BUCKETS, load_feature_stats
...
def load_model_state_strict(
    ...
    # --- 新增：处理 torch.compile 产生的前缀 ---
    new_state_dict = {}
    for k, v in state_dict.items():
        name = k.replace("_orig_mod.", "") # 移除编译前缀
        new_state_dict[name] = v
    
    try:
        # 先尝试非严格加载，方便观察具体哪个 key 出错
        missing, unexpected = model.load_state_dict(new_state_dict, strict=False)
        if missing or unexpected:
            logging.warning(f"Weights partially matched. Missing: {len(missing)}, Unexpected: {len(unexpected)}")
            if missing:
                logging.error(f"First few missing keys: {missing[:5]}")
        
        # 如果你确定结构必须完全一致，可以保留这里的校验或直接改用 strict=False 
        # model.load_state_dict(new_state_dict, strict=True) 
        
    except RuntimeError as e:
        logging.error("Failed to load state_dict. Check model architecture vs checkpoint.")
        raise e

def _find_pt_file(directory: str) -> Optional[str]:
...
def _checkpoint_dir_rank(directory: str) -> Tuple[int, int, float]:
    name = os.path.basename(directory)
    best = 1 if name.endswith('.best_model') else 0
    ...
    try:
        ...
        mtime = 0.0
    return best, step, mtime


def resolve_model_dir_and_ckpt(model_output_path: str) -> Tuple[str, str]:
    """Resolve platform ``MODEL_OUTPUT_PATH`` to ``(ckpt_dir, model.pt)``.

    Some environments point ``MODEL_OUTPUT_PATH`` at the selected
    ``global_step...`` directory, while others expose the model root that
    contains multiple ``global_step...`` subdirectories. Support both.
    """
    if os.path.isfile(model_output_path) and model_output_path.endswith('.pt'):
        return os.path.dirname(model_output_path), model_output_path

    direct = _find_pt_file(model_output_path)
    if direct is not None:
        return model_output_path, direct

    candidates: List[str] = []
    for dirpath, _, filenames in os.walk(model_output_path):
        if any(name.endswith('.pt') for name in filenames):
            candidates.append(dirpath)
    if not candidates:
        raise FileNotFoundError(
            f"No *.pt file found under MODEL_OUTPUT_PATH={model_output_path!r}. "
            f"The directory contains: {os.listdir(model_output_path) if os.path.isdir(model_output_path) else 'N/A'}."
        )

    candidates.sort(key=_checkpoint_dir_rank, reverse=True)
    ckpt_dir = candidates[0]
    ckpt_path = _find_pt_file(ckpt_dir)
    if ckpt_path is None:
        raise FileNotFoundError(f"Internal error: selected {ckpt_dir!r} but no *.pt file was found.")
    logging.info(f"Resolved checkpoint directory: {ckpt_dir}")
    return ckpt_dir, ckpt_path


def load_feature_stats_for_infer(
    model_dir: str,
    train_config: Dict[str, Any],
) -> Optional[Dict[str, Any]]:
    use_engineered = bool(train_config.get('use_engineered_features', False))
    groups = str(train_config.get('engineered_feature_groups', ''))
    needs_stats = use_engineered and 'item' in {g.strip() for g in groups.split(',')}
    if not needs_stats:
        return None
    ...
    if not os.path.exists(stats_path):
        raise FileNotFoundError(
            f"Engineered item-stat features were enabled during training, but "
            f"feature_stats.pkl was not found in MODEL_OUTPUT_PATH={model_dir!r}. "
            "Use a checkpoint saved after the feature-stats sidecar was added, "
            "or disable item engineered features and retrain.")
    logging.info(f"Loaded feature stats from {stats_path}")
    return load_feature_stats(stats_path)
...
def main() -> None:
    ...
    model_dir, ckpt_path = resolve_model_dir_and_ckpt(model_dir)
    ...
    test_dataset = PCVRParquetDataset(
        ...
        buffer_batches=0,
        is_training=False,
        enable_engineered_features=use_engineered_features,
    ...
    predictions = {
        "predictions": {str(uid): float(prob) for uid, prob in zip(all_user_ids, all_probs)},
    }
